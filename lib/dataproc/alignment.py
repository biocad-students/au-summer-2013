...
# Alignment pattern on sequence using regex
def align_pattern_on_fasta(fasta, pattern):
    for record in fasta:
        seq = record.seq
        find_result = re.search(pattern, seq)
        region = find_result.group(0)
        if record:
            record.seq = region
            yield record
...
class Aligner:
    ...
    def edit_dist(self, seq1, seq2):
        ...
        #     child = subprocess.Popen(cmdline, stdin=subprocess.PIPE,
